...
def task(line_pass: tuple,task_name:str='',number_task:int=2):
    line_value_dict = {
        #'task_number': 'serial',
        'sat_name': line_pass[1],
        'aos': line_pass[2],
        'los': line_pass[3],
        'sat_id': line_pass[0],
        'station_id': line_pass[5],
        'status': 'waiting'
    }
    ...
    duration=int(configure.get(task_name, 'duration'))


    if AB == 'befor':
        task_time_start=line_value_dict['aos']- timedelta(minutes=delta_time)
        task_time_end = line_value_dict['los'] + timedelta(minutes=delta_time)

    elif AB=='after':
        task_time_start = line_value_dict['los'] + timedelta(minutes=delta_time)
        task_time_end  =  task_time_start +  timedelta(minutes=10)
        #print(type(task_time_start) , type(timedelta(minutes=delta_time)) , type (line_value_dict['los'] ))
...
coonn = connexion()
curr = coonn.cur
sql=(f'''SELECT distinct * FROM groundstation.planning_pass
where planning_pass."AOS" > now() at time zone 'utc'
order by  planning_pass."AOS" 
''')
